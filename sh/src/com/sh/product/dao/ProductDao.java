...
public class ProductDao {
	...
	public List<Product> findAllHot(){
		try{
			System.out.println("dao..........");
			Query query=this.sessionFactory.openSession().createQuery("from Product p where p.hhot = ?");
	...
	public List<Product> findAllNew(){
		try{
			System.out.println("dao..........");
			Query query=this.sessionFactory.openSession().createQuery("from Product p where p.nnew = ?");
			query.setInteger(0, 1);
			List<Product> list = query.list();
			for(Product p:list){
				System.out.println(p);
